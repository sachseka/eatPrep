...
test_that("optional numeric cuts retain exact values and preserve estimates", {
  ...
  expect_true(all(marks$x > 0 & marks$xend > marks$x))
...
test_that("crowded maps reserve a separate cut column on different devices", {
  ...
  for (geom in c("density", "histogram")) {
    ...
    for (size in list(c(6, 8), c(10, 5))) {
      ...
      tryCatch({
        ...
        expect_lt(labels[[1L]]$right, min(labels[[2L]]$anchor))

...
The functions creates and saves those file, but the actual return values for this function is `NULL`.
...
`prep2GADS` converts the data frame that we prepared to an eatGADS object for further use in the packages `eatGADS`. It uses the meta data stored in `inputList`, so it needs the `inputList` as an argument, as well as our data frame `dat`, that we have been preparing.

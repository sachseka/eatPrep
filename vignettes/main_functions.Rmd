...
idm1 <- c("[computeCutsIDM()](#idm-cut-scores)", "computes cut scores for Item Descriptor Matching based on monotonized moving averages.")
idm2 <- c("[plotCutsIDM()](#idm-cut-scores)", "plots the raw ratings, moving averages, monotonized curves and cut scores from `computeCutsIDM()`.")
...
Cut scores are interpolated boundary crossings of the monotonized curves. `cuts_per_person` and `cuts_summary` report these cuts on the item difficulty scale. `cut_positions_per_person` stores the corresponding interpolated item positions, while `cut_statistics` and `level_statistics` summarize cuts and item difficulty intervals in the style often used in IDM documentation.

`plotCutsIDM()` visualizes the raw ratings, moving averages, monotonized curves and resulting cut scores.

```{r idm plot example, fig.width=7, fig.height=5}
plotCutsIDM(cuts)
```

An aggregate panel can be added to inspect all monotonized rater curves together with the mean cuts.

```{r idm aggregate plot example, fig.width=7, fig.height=5}
plotCutsIDM(cuts, show_aggregate = TRUE)
```

Residual panels can be added to inspect local deviations between raw ratings and the smoothed moving average.

```{r idm residual plot example, fig.width=7, fig.height=6}
plotCutsIDM(cuts, show_residuals = TRUE)
```

...
Within a PV, SD is the square root of the weighted mean squared deviation from that PV's weighted mean, with weights normalized to sum to one. This is a descriptive population SD, without an `n - 1` sample-variance correction. SD describes population spread; it is not a standard error. For example, with weights `c(1, 3)`, PV1 values `c(0, 4)` have M = 3 and variance = 3, while PV2 values `c(18, 10)` have M = 12 and variance = 12. The displayed estimates are therefore M = 7.50 and SD = sqrt((3 + 12) / 2) = 2.74.

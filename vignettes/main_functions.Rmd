...
Both functions estimate a Gaussian kernel density separately for each PV, normalizing the available sampling weights to sum to one within each PV. They then average the densities with equal weight across PVs on a common 512-point grid. **Do not average a respondent's PVs first**: the distribution of respondent averages would discard within-respondent PV variation. This follows the principle of computing population estimates separately for each PV and averaging those estimates, described in the [OECD PISA analysis guidance](https://www.oecd.org/en/about/programmes/pisa/how-to-prepare-and-analyse-the-pisa-database.html).
...
Both plot functions now show population percentages by default when PV data are supplied. Each panel has a small table above its data: columns `[1]`, `[2]`, and so on refer to cut intervals from left to right, with a separate colored row for each population in legend order. For one ungrouped population, the table has a single neutral row.
...
For a panel with two cuts `c1` and `c2`, the table columns mean:

| Column | PV values included |
|:--|:--|
| `[1]` | Below `c1` |
| `[2]` | At or above `c1`, but below `c2` |
| `[3]` | At or above `c2` |
...
Table columns have equal display width so that close or identical cuts do not cause percentage labels to overlap. The column numbers identify interval order, **not rating stages or score-axis positions**. The table should therefore be read as a table, not as labels located at the interval midpoints. With many cuts or a narrow plot, reduce `percentage_size` or increase the figure width.

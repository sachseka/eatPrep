...
After importing the data and making sure it has the right format, the data needs to be recoded. For that, you need recode data sets with special consideration of missing values. See `collapseMissings()` for supported types of issing values. `recodeData()` recodes those data frames and will give warnings, if missing or incomplete recode information is found. Values without recode information will not be recoded. 
...
```{r recodeData}
datRec <- recodeData(dat = inputDat[[1]], values = inputList$values, subunits = inputList$subunits)
```

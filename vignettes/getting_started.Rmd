...
Here you see an overview over the main functions of `eatPrep`. If you want to learn more about using this package for IRT analysis, this [vignette](main_functions.html) explains how to use these functions.
...
head6 <- c("**Export**", "")
# 1
read1 <- c("[readDaemonXlsx()](main_functions.html#items-via-zkdaemon)", "read in the inputlist that was created using the EDV-tool 'ZKDaemon'.")
read2 <- c("[readSpss()](main_functions.html#spss-data)", "read in SPSS files.")
read3 <- c("[readMerkmalXlsx()](main_functions.html#merkmalsauszug)", "read in additional item and exercise attributes like processing time, formats, content categories, ...")
# 2
check1 <- c("[checkInputList()](main_functions.html#checkinputlist)", "check the inputList for internal consistency.")
check2 <- c("[checkData()](main_functions.html#checkdata)", "check data sets according to item meta information and other plausibility checks of the data.")
check3 <- c("[checkDesign()](main_functions.html#checkdesign)", "check data sets according to test design meta information.")
# 3
merge1 <- c("[mergeData()](main_functions.html#merging-data)", "merging the data sets and diagnostics to ensure a fit.")
recode1 <- c("[recodeData()](main_functions.html#recoding-data)", "recode the subitems according to meta information from the inputList.")
aggregate1 <- c("[aggregateData()](main_functions.html#aggregating-data)", "aggregate subitems into items.")
score1 <- c("[scoreData()](main_functions.html#scoring-data)", "recode items that previously consisted out of subitems.")
score2 <- c("[mnrCoding()](main_functions.html#mnrcoding)", "recoding the last items (if empty) in each block (see test design) as 'missing not reached'.")
# 4
wrap1 <- c("[automateDataPreparation()](main_functions.html#wrapper)", "wraps most of the other features into one big function.")
# 5
pbc1 <- c("[catPbc()](main_functions.html#category-discrimination)", "calculates category-level point-biserial correlations for recoded and raw item data.")
pbc2 <- c("[evalPbc()](main_functions.html#category-discrimination)", "flags suspicious category discrimination patterns from the output of `catPbc()`.")
rater1 <- c("[meanAgree()](main_functions.html#rater-agreement)", "calculates mean pairwise percentage agreement across raters.")
rater2 <- c("[meanKappa()](main_functions.html#rater-agreement)", "calculates mean pairwise kappa across raters.")
rater3 <- c("[make.pseudo()](main_functions.html#pseudo-raters)", "reduces multiple real raters to a smaller number of pseudo raters.")
idm1 <- c("[computeCutsIDM()](main_functions.html#idm-cut-scores)", "computes cut scores, modal values, and agreement diagnostics for Item Descriptor Matching based on monotonized moving averages.")
idm2 <- c("[plotCutsIDM()](main_functions.html#idm-cut-scores)", "plots the raw ratings, moving averages, monotonized curves, optional labelled aggregate or residual panels, and cut scores from `computeCutsIDM()`.")
visual1 <- c("[visualSubsetRecode()](main_functions.html#visual-subset-recoding)", "supports interactive visual inspection and recoding of flagged subsets.")
# 6
export1 <- c("[collapseMissings()](main_functions.html#collapsing-data)", "recodes missing types into predefined scores (usually 0, 1, and NA). Such a collapsed R data frame can be passed directly to eatModel for scaling.")
export2 <- c("[writeSpss()](main_functions.html#export-spss)", "produces an SPSS syntax and a .txt data set that can be read into SPSS with the syntax including all meta data.")
export3 <- c("[prep2GADS()](main_functions.html#prepare-for-eatgads)", "both the raw data sets and the finished, scored data sets, including all their meta data, can be exported into a GADSdat object for data storage or further processing in eatGADS.")

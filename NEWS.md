...
* `plotWrightMap()` gains optional mean cuts in a separate column on the right. Supply a numeric vector, a one-row `cuts_summary` table, or a `computeCutsIDM()` result (only its mean cuts are used). Short horizontal marks retain exact scores, while labels avoid overlap. Cut names, value display, rounding, text size, and colour are configurable. Unrounded values remain available in `attr(plot, "wright_data")$cuts`.
